...
load(here("3.Model_Selection/3.1.CHANGE/3.1.2.HIC_CHANGE/HIC_Define_Models_input.RData"))
...
globmod_list5 <- HIC_globmod_list[1:2]
## output --> result_list5
...
## input --> globmod_list_....
globmod_list3 <- HIC_globmod_list[3:length(HIC_globmod_list)]
## output --> result_list3

# The function combines the all the steps at once (step 2- dredge, 3- creates the model average, 4- handles model average. 5- calculate importance by sw)
fun_dredge_avg_sw3 <- function(globmod_list3, mod_dredge, mod_avg,sum_mod_avg, df_sum_mod_avg, importance_sw ){
  # # 2. Dredge
...
# Combine the data frames generated in each model into a single data frame
df_sum_mod_avg_3 <- do.call(rbind, sum_mod_avg_list3)
...
result_list_all_HIC <- c(result_list5, result_list3)

## BEST Models ----
# 1. Check the global model calls and report
# # Find the null model (assuming a column 'model' contains the model formulas)
#null_model_index <- which(grepl("^~ 1$", result_list_all_HIC[[1]][["mod_dredge"]]$model))
for (i in 1:3){
print(result_list_all_HIC[[i]][["mod_dredge"]])
}
...
for (i in 1:length(result_list_all_HIC)) {
  ...
  best_mod[i] <- MuMIn::get.models(result_list_all_HIC[[i]]$mod_dredge, subset = 1)
  print(best_mod[[i]]@call)
}
lmer(formula = change ~ meantmpAreaPop + workTotal + x0008 + 
       (1 | ISO3) + (1 | SHORTNAME), data = dframe.iS)
#! Error in h(simpleError(msg, call)) : 
#!  error in evaluating the argument 'x' in selecting a method for function 'print': no applicable method for `@` applied to an object of class "lm"
#!fix this error manually
#2. check the the difference between delta of null model and best selected model. 
#If delta is bigger than absolut 2 report on the excel "new_income dpsir model selection formulas". 

View(result_list_all_HIC[[1]][["mod_dredge"]])

for (i in 1:length(result_list_all_HIC)){
  print(MuMIn::get.models(result_list_all_HIC[[i]][["mod_dredge"]], subset = 1))
}
#for coef estimates
HIC_globmod_coef_est_results <-rbind(df_sum_mod_avg_5, df_sum_mod_avg_3)
#for importance
HIC_globmod_importance_results <-rbind(importance_sw_df5, importance_sw_df3)

#save results
write_csv(HIC_globmod_coef_est_results, file= here("3.Model_Selection/3.1.CHANGE/3.1.2.HIC_CHANGE/HIC_globmod_coef_est_results.csv"))
write_csv(HIC_globmod_importance_results, file= here("3.Model_Selection/3.1.CHANGE/3.1.2.HIC_CHANGE/HIC_globmod_importance.csv"))
# 
# #save workspace
save.image(file =here("3.Model_Selection/3.1.CHANGE/3.1.2.HIC_CHANGE/", "HIC_global_workspace_output.RData"))
